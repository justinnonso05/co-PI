...
export default function CollaborativeEditorPage() {
  ...
  const acceptAiSuggestion = () => {
    if (quillRef.current && aiSuggestionRef.current) {
      ...
      // Get the exact text generated by the AI
      const text = quillRef.current.getText(index, length);
      
      // Silently remove the green "preview" text
      quillRef.current.deleteText(index, length, 'api');
      
      // Re-insert it as a 'user' action so the delta broadcasts to all collaborators instantly
      quillRef.current.insertText(index, text, 'user');
    }
